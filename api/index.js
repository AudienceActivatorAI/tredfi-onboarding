...
var onboardingSubmissions = mysqlTable("onboarding_submissions", {
  id: int("id").autoincrement().primaryKey(),
  // Dealership info
...
var appRouter = router({
  ...
  onboarding: router({
    submit: publicProcedure.input(z2.object({
      dealershipName: z2.string().optional(),
      dealershipAddress: z2.string().optional(),
      dealershipPhone: z2.string().optional(),
      primaryContactName: z2.string().optional(),
      primaryContactEmail: z2.string().email().optional(),
      primaryContactCell: z2.string().optional(),
      crmName: z2.string().optional(),
      crmNotApplicable: z2.boolean(),
      dmsName: z2.string().optional(),
      dmsNotApplicable: z2.boolean(),
      websiteProvider: z2.string().optional(),
      websiteNotApplicable: z2.boolean(),
      thirdPartyVendors: z2.string().optional(),
      thirdPartyNotApplicable: z2.boolean(),
      facebookAdsUsage: z2.string().optional(),
      facebookAdsNotApplicable: z2.boolean(),
      marketplacePlatforms: z2.string().optional(),
      marketplaceNotApplicable: z2.boolean(),
      backendProducts: z2.string().optional(),
      backendNotApplicable: z2.boolean(),
      subprimeLenders: z2.string().optional(),
      subprimeNotApplicable: z2.boolean(),
      salesProcessStructure: z2.string().optional(),
      salesProcessNotApplicable: z2.boolean(),
      rehashingLenders: z2.string().optional(),
      rehashingNotApplicable: z2.boolean(),
      platformName: z2.string().optional(),
      colorScheme: z2.string().optional(),
      tireWheelSales: z2.string().optional(),
      platformUsage: z2.string().optional()
    })).mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) {
    ...
    generateNames: publicProcedure.input(z2.object({
      dealershipName: z2.string(),
    ...
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      const submissions = await db.select().from(onboardingSubmissions).orderBy(desc(onboardingSubmissions.submittedAt));
      return submissions;
    }),
    updateStatus: protectedProcedure.input(z2.object({
      id: z2.number(),
      status: z2.enum(["new", "in_progress", "completed"]),
      ...
      await db.update(onboardingSubmissions).set({
        status: input.status,
        notes: input.notes
      }).where(eq2(onboardingSubmissions.id, input.id));
      return { success: true };
    })
  })
});
